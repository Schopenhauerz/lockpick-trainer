...
const SWEET_SPOT_END = 0.99;    // sweet spot always ends at 99% of track
...
export function useGameEngine() {
  ...
  const [config, setConfig] = useState({
    speedMultiplier: 4.0,
    inputLatency: 0,
    sweetSpotWidth: 0.19,  // 80-99% window
    showSweetSpot: false,
  });
  ...
  const evaluate = useCallback((key, capturedProgress) => {
    ...
    const activeWidth    = configRef.current.showSweetSpot ? configRef.current.sweetSpotWidth : 0.19;
